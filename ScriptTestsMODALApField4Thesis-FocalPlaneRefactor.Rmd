...
```{python}
myrep = '/home/james/GRASPdata/MyTabSourceFIModel/150GHz/MODALfiles/'
...
print(qbrepDONNY)
print(mrep)
reps = np.array([myrep, qbrepDAVE, qbrepDONNY, mrep])
```
...
plt.scatter(data['Xpos'], data['Ypos'], c=gaussit, cmap='gnuplot', marker='.')#, s=1)
...
ints = np.array([myit, daveit, donnyit, gaussit])
...
for i, rep in enumerate(reps):
    #print(i)
    plt.plot(aziA[i,:], ziA[i,:], label='Model {}'.format(reps[i]))
...
for i, rep in enumerate(reps):
    #print(i)
    plt.plot(aziA[i,:], ziA[i,:]/max(ziA[i,:]), label='Model {}'.format(reps[i]))
plt.legend()    
#plt.yscale('log')
plt.figure(figsize=(16,8))
for i, rep in enumerate(reps):
